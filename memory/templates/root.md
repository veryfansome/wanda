...
Record what is true, greedily. Anyone named, anywhere they were, any organisation, anything that happened: write it down. Deciding what matters is not a decision that has to be made now, and a fact that was not recorded cannot be recalled later.
...
Read the index in a directory before reading the files in it. Reading any file in a directory also brings that directory's index into the session's context, so navigate toward what is needed rather than reading widely.

When something recorded turns out never to have been true, invoke the `retract` skill rather than writing the correction beside it. A claim left standing next to its correction is still there to be recalled, and both will come back — and a claim that reached a node's summary is still in every index after the edge that carried it is gone.
...
`mem` is how the graph is read and written. Run `mem help` for the full list. The ones that will be wanted:
...
Where a flag names who, what or where something involves — `--participants`, `--place`, `--about`, `--whose`, and both ends of `relate` — a name that is not already a node becomes a new one, so pass an id there where one is known. `entity`, `event`, `trajectory` and `pref` take `--body` for what the summary does not hold. An event that happened on a day other than today takes `--when <YYYY-MM-DD>`: the date is the front of its id, and with the summary it decides whether this is a new event or one already here. `mem <verb> --help` has each verb's flags.
