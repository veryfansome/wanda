...
1. **Install**: `uv sync` — wanda runs from this checkout: `prompts/` and `skills/` are read from the repo root at runtime and are deliberately not part of the wheel.
...
Each directory has a `CLAUDE.md` write-spec — what belongs there, how it is named, when to split. wanda reads them when filing and revises them from the preference claims recorded in `prefs/` — yours, or ones a session concluded, never email content — and reports every change; set `WANDA_MEMORY_WRITESPEC_OWNER_ONLY=1` to require your word. The workspace `CLAUDE.md` every session loads is composed from the root write-spec plus generated blocks (standing rules, due soon, who is in play), capped at 4 KB.
...
Residuals, stated plainly. The softer session-vs-email distinction for *hand-written* vault content (a claim a session types directly into a note, with no ledger evidence) rests on the same trust as an Obsidian edit — wanda treats it as the owner's word and it becomes session-tier, so a session with shell access can get its text into curated notes and the projection tagged `[noted]`. It still cannot reach owner tier or state a disposition. Run-window records (which attribute each shell-written line to email or session tier) live in `wanda.db`, which a session can edit; a session that tampers with them can flip its own hand-written content between email and session tier — again, never to owner and never to a disposition, and the running daemon re-derives from its own in-memory copy. The tool-call log is tamper-evident rather than tamper-proof — a session can edit its own log, which is why lines are mirrored to the unified log. A forged `attest` can still borrow a genuine attest message if the same session also rewrites the claim it names — the note edit is reported as a hand-edit, the forged line is not. A pending rule offer lives in `wanda.db` too: a session that rewrites the row changes what `rule kN` mints, which is why accepting one echoes the text back — trust the echo, not the offer id. Repairing a cleared owner-check row works only while the daemon still holds that line's authority in memory, so a rule it has not re-verified since its last restart can still be demoted by editing `wanda.db`, until the next successful re-check. An `ids:` line in a note's frontmatter is read as written and nothing checks who added it, so a session can point an address at a note it chose, and that note's title is what the triage block shows for that sender.
