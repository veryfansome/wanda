...
- Agent sessions run in `~/.wanda/workspace` with `Bash,Read,WebSearch,Skill` and `--setting-sources project` (which is what loads the skills). They are **not** sandboxed — see [Trust assumption](#trust-assumption) above for what that means and how to lock it down. `WANDA_SLACK_OWNER_USER_IDS` restricts who can trigger them; empty means anyone in the workspace.
