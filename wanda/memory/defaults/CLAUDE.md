...
- `prefs/` — how Alex wants things handled. One policy lives once here and points at the senders it governs; never copy a policy onto a sender's note.
...
Claims go between the `wanda:begin claims` and `wanda:end claims` markers. Everything under `## Notes` is Alex's and is never touched. Every claim carries the edges that justify it, so "why does wanda think this" is always one hop away.
