...
`refs_unresolved` — recalled refs in which the judge finds no node, listed at the foot of the report. Every id written in a ref is read unless a letter or digit touches it, and a bare six characters that two stored nodes end in names neither, so what is left names no node by id or by name: a session or a `mem session` call, an index, an event named by its date alone, or prose. It matters because an unresolved ref is scored without the node's body, which biases toward miss — and a sudden jump against the sibling runs is a reason to check that the right store was materialised before concluding anything about the sessions.
...
A `session` call can differ from what the run printed without anything being wrong: a rebuild sees every transcript of the run, later arrivals included; and replaying a log from before round 20, the caller's own exchange is left out of a listing and no longer shown as in progress by its id, and a `--day` given as the clock's date is read as the story's. The store is not affected, because `session` only reads.
