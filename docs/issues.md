...
Round 20, read by that rule, did not move: no first person in any run, and every run's second-half no-actor share was 0%. The sessions whose `mem session` listing changed read the same as the rest, and one session in the round cited "own name". Being told it is wanda does not change how a session files its own acts.

Round 21 puts everything wanda reads in the first person — the vault's standing texts and skills, the prompt ("I am wanda."), the arrival frames, a paragraph in the system prompt saying whose "I" it is, everything `mem` prints, her node (labelled `me`, "my name is wanda", `--about me`, with "wanda" resolving to it), and the product's prompts and skills — and enrich step 5's forms write her own acts with a literal "I". It carries that change alone. `runs/measures21/README.md` says how it is read against round 20: the voice in summaries and, apart from the forms, in bodies, asides and answers; the self node; and guards on everything else the rewrite could move.

Round 21 moved the voice all the way: 65 of 65 own-act summaries are in the first person, against 0 of 76 in round 20, and none names wanda. The rules of conduct gained force with it — answers describing the filing 46 → 16, non-checkpoint arrivals answered 116 → 74, exchange nodes 14 → 7 — while the sentences that name a means, or a step to take every time, lost it: sessions calling `mem recall` 436 → 293, enrich where nothing was written 50 of 76 → 12 of 93, idle sessions 26 → 81, sessions with six or fewer tool calls 26 → 61. At two greeting checkpoints, hits on the preference the item asks about went from 8 of 8 to 1 of 8, in sessions that stopped before they reached it. The reading, which is not measured: the first person turned the rules into her character, so dispositions strengthened and routines became habits, applied only where their point seemed live.

**Fix.** Round 22 gives every procedure sentence — a tool or means to use, or a step to take every time or under a trigger — as round 20's wording with the pronoun removed, a bare imperative or the passive: in the prompt, the vault's standing texts, the two index lines `mem` writes that say how to navigate, and the product's prompts and skills. Who she is, how she speaks, the Discretion section and enrich step 5 stay in her first person, everything else is as round 21 has it, and no paragraph or list item holds both an imperative and a first-person word. It carries that change alone. `runs/measures22/README.md` says how it is read against round 21, with rounds 19 and 20 as the level the procedures fell from.
