...
**No deployment has run yet — there is no vault, no `wanda.db`, no ledger history.** So none of these items carries a migration constraint, and each should take the simplest correct shape rather than the most backward-compatible one: drop a field rather than clearing it, delete a verb rather than deprecating it, change a stored format rather than reading both. Anything below that reads as cautious about existing data is over-specified; ignore that part.
...
| A3 | **Do not store `messages.snippet` at all.** Carry the body through the triage batch in memory and drop the column. | `store.py`, `imap_watcher.py` | After A1+A2 the only reader is `triage.py`, in the same cycle as the fetch. A1 must land first: `apply_row` re-runs `post_task` on recovery, so the Slack post has to stop needing the body before the body stops existing. |
...
| B4 | **Re-mint on message edit.** An owner editing their own Slack command currently invalidates the rule it minted permanently; nothing consults Slack's `edited`/`subtype` signal. | `passes.py` | **DECIDED:** re-mint from the new text, and retire the claim the old line produced. Partly mitigated already by `41bea60`: an edit that leaves the message no longer parsing as a command yields NO_CANDIDATES, so the rule is no longer quarantined as a forgery and the owner is told to re-state it. What remains is the edit that still parses — today it silently invalidates the line it minted. Constraint: the ledger is append-only (`ledger.append` is the only writer), so the old *line* cannot be deleted — only the claim it produced can be retired. |
...
| # | Change | Files | Note |
|---|---|---|---|
| C1 | **Remove `unretire`** — CLI verb, Slack verb, restore path, journal handling. Recovery is "tell wanda again, mint fresh." | `memory_cli.py`, `commands.py`, `passes.py` | **DECIDED.** Deletes the Z12 bug class (unretire-after-`retire --to` producing two notes with one `ids:` set) outright. A plain deletion: no retired notes exist to strand, and `retired/` can go with it. Also update `README.md`, `SKILL.md`, `defaults/README.md`. |
...
| # | Change | Note |
|---|---|---|
| E1 | Subsumed by A3: with the column gone there is nothing to retain. Noted only because the snippet is the one `G6` candidate that is **not a trust input**, so it was always the easy one. | The only prunes in the tree are `store.prune_slack_events(7)` and `audit.prune(90)`; neither touches `messages`. |
