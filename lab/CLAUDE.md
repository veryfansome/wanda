...
- **The simulated date goes in `--append-system-prompt`.** In the user prompt it reads as something the user believes and loses to the real date, which reaches the session as an attachment on the first user turn. `mem` also rewrites the real date out of every string a session passes, at one choke point after parsing — an exact match on today and nothing near it, because a near-miss window caught the story's own dates instead. `corpus.py` shifts the history by whole weeks and always lands it short of the anchor, never level with it, so a date equal to the system date was read off the clock rather than taken from the story. Claude Code presents the user turn, `CLAUDE.md`, skills and a command's output as someone else's words, so ahead of the date the appended prompt says who "I" is. The product gives its sessions the same paragraph, appended to Claude Code's prompt for her agent sessions and opening the triage prompt that replaces it, so it changes in both or neither. Who she is, how she speaks and what she herself said and did are in her first person, in the prompt, the vault's texts and what `mem` prints, including `mem`'s help lines where the vault's texts quote them and the steps that belong to those, such as enrich's step 5. Every other step the prompt and the vault's texts give, with the means named for it, has no pronoun: it is an imperative or in the passive. No paragraph holds both an imperative and a first-person word, counting each list item and each entry of a command listing as a paragraph of its own.
...
- **`run` checks at startup that the projection reads back what the prompt writes, and every prompt it wrote before.** The prompt a session is handed and the parser that reads it out of a transcript are one shape written twice; edit them together, or the harness refuses to start. When the prompt or a frame is replaced, the prompt as it was goes into `EARLIER` in `arrival.rs` with the frames it was handed with, both written out whole as sessions were handed them, because `mem session` still reads the transcripts that hold them.
- **The path is the id.** `events/2026-09-07-e6799d.md` is `event:2026-09-07-e6799d` — the directory is the kind, the stem is the rest, and nothing in the file repeats either. Ids are opaque and minted at creation; names are labels that resolve by lookup, and a name two nodes carry is refused with the candidates shown. Her own two names, `me` and `wanda`, are the exception: they always find her node, and a namesake is reached by its id.
